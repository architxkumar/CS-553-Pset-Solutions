...
class Open:
    ...
    def __init__(self, initial_state):
        Open.stack.append(initial_state)
        Open.index += 1
    ...
    def pop():
        ...
        Open.index -= 1
        return Open.stack.pop()

    @staticmethod
    def push(new_state):
        Open.index += 1
        Open.stack.append(new_state)
...
def main():
    ...
    while open_stack.index != -1:
        ...
        if matrix == solution:
            ...
            return
        close.append(matrix)
        matrix_heuristic_value = heuristic(matrix)
        generated_nodes = generate_node(matrix)
        for node in generated_nodes:
            heuristic_value_node = heuristic(node)
            # Check: To ensure that previously explored nodes aren't pushed into the stack
            if node not in close and heuristic_value_node >= matrix_heuristic_value:
